...
class RosNet:
    ...
    def __init__(self, x, n_classes):
        ...
        height, width = self.x.shape[0], self.x.shape[1]
        self.shape = [1, height, width, 3]

    def convnet(self):
        rv = self.get_rv(self.shape)
        with tf.name_scope('rosnet'):
            ...
            with tf.name_scope('fcl_layer'):
                with tf.name_scope('flatten'):
                    fclr = tf.reshape(conv2_maxpool2d, [tf.shape(self.x)[0], rv*rv*64])
                with tf.name_scope('ReLU_add_matmul'):
                    fcl = tf.nn.relu(tf.add(tf.matmul(fclr, init_weights('weights', 'fcl', [rv*rv*64, 128])),
